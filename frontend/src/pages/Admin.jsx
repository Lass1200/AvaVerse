...
export default function Admin({ session }) {
  ...
  async function fetchAdminAvatars(token) {
    const response = await fetch(`${API_URL}/api/admin/avatars?status=pending`, {
      method: 'GET',
  ...
  if (!session.isAdmin) {
    return (
      <main className="register-page">
        <Header session={session} adminTab={adminTab} setAdminTab={setAdminTab} />
...
function Header({ session, adminTab, setAdminTab }) {
  return (
    <header className="site-header admin-header">
      ...
      <nav className="top-nav" aria-label="Navigation administrateur">
        <button
          className={adminTab === 'moderation' ? 'active' : ''}
          type="button"
